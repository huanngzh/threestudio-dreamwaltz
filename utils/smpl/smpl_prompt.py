...
class MySMPL(object):
    def __init__(
        self, batch_size, model_type, model_path=SMPL_ROOT, gender="neutral"
    ) -> None:
        # Build SMPL Model
    ...
    def __call__(self, body_pose=None, random_pose=True, **kwargs):
        ...
        # Select keypoints
        keypoints = joints[
            ..., smpl_to_openpose(self.model_type, openpose_format="coco18"), :
        ]
        # Normalize
...
class _HumanScene(object):
    ...
    def export_pose_map(
        self, intrinsics, extrinsic, width=512, height=512, occlusion_culling=True
    ):
        """
        Input:
            intrinsics: np.array, [3, 3]
    ...
    def detect_occlusion(self, CAM_world, thres=0.02):
        ...
        CAM_world = np.broadcast_to(
            CAM_world.T, (*(KP_world.shape[:-1]), 3)
        )  # [N, 18, 3]
...
class RandomScene(_HumanScene):
    def __init__(self, num_person=1, model_type="smpl", spacing=0.8, **kwargs) -> None:
        ...
        transl_pattern = {
            ...
            3: torch.tensor(
                [[0.0, 0.0, +spacing], [-spacing, 0.0, 0.0], [+spacing, 0.0, 0.0]]
            ),
            4: torch.tensor(
                [
                    [+spacing, 0.0, +spacing],
...
class AnimatedScene(_HumanScene):
    def __init__(
        self, scene, num_person=1, model_type="smpl", pop_transl=True, **kwargs
    ) -> None:
        super().__init__(model_type=model_type, **kwargs)
        # Load data from dataset
        self.scene = scene
        if scene not in ("random", "rand"):
            self.smpl_seqs, self.num_person, self.num_frame = self.load_smpl_sequences(
                scene, num_person=num_person, model_type=model_type
            )
        else:
    ...
    def load_smpl_sequences(self, scene, num_person, model_type):
        ...
        if len(scene_args) > 1:
            ...
            frame_interval = (
                tuple(map(int, frame_interval.split("-")))
                if "-" in frame_interval
                else eval(frame_interval)
            )
        else:
            frame_interval = None
        # Load Data
        ...
        identity_idx = None
        smpl_seqs = self.preprocess_smpl_sequences(
            smpl_seqs, identity_idx=identity_idx, frame_interval=frame_interval
        )
    ...
    def preprocess_smpl_sequences(
        self, smpl_seqs, identity_idx, frame_interval, to_tensor=True
    ):
    ...
    def set_frame_index(self, frame_idx):
        ...
        if "transl" in smpl_params.keys():
            if self.num_person == 1:
                ...
                smpl_params["transl"] -= torch.mean(
                    transl, dim=0, keepdim=True
                )  # [N, 3] - [1, 3]
        # Set SMPL Params
        smpl_params = self.add_offset_to_smpl_params(smpl_params)
        self.smpl_params = smpl_params
...
class SMPLPrompt:
    def __init__(
        ...
        # Build Scene
        if scene == "random":
            self.hs = RandomScene(num_person=num_person, offset_y=smpl_offset_y)
    ...
    def __call__(self, intrinsics, cam2world, cond_type=None, frame_idx=None):
        ...
        for _cond in cond_type:
            if _cond == "pose":
                cond_image = self.hs.export_pose_map(
                    intrinsics, extrinsic, width=self.width, height=self.height
                )
            elif _cond == "depth":
                cond_image = self.hs.export_depth_map(
                    intrinsics, extrinsic, width=self.width, height=self.height
                )
            elif _cond == "mesh":
                cond_image = self.hs.export_mesh_map(
                    intrinsics, extrinsic, width=self.width, height=self.height
                )
            else:
                assert 0, _cond
...
class AnimatedSMPLPrompt:
    def __init__(
        ...
        if "canonical" in scene:
            ...
            self.hs = AnimatedScene(
                scene=scene,
                num_person=num_person,
                offset_y=smpl_offset_y,
                pop_transl=pop_transl,
            )
            self.num_person = self.hs.num_person
            self.num_frame = self.hs.num_frame
        # Canonical Scene Initialization
        self.hs_canonical = CanonicalScene(scene="canonical-A", offset_y=smpl_offset_y)
        # Others
    ...
    def write_video(
        ...
        for i in range(num_frames):
            intrinsics, cam2world = index2pose(
                i, H=self.height, W=self.width, radius=radius, camera_track=camera_track
            )
            image = self(intrinsics, cam2world, cond_type=cond_type, frame_idx=i)[
                "cond_images"
            ][0]
            if i == 0:
                image.save(osp.join(save_dir, save_image))
