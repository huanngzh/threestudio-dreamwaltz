...
def rand_pose(
    ...
    up_vector = safe_normalize(
        torch.cross(right_vector, lookat_vector, dim=-1) + up_noise
    )
...
def circle_pose(
    device, radius=1.25, theta=80, phi=0, return_dirs=False, view_prompt=None
):
    ...
    right_vector = safe_normalize(
        torch.cross(
            lookat_vector, torch.FloatTensor([0, -1, 0]).to(device).unsqueeze(0), dim=-1
        )
    )
...
def index2pose(
    ...
    phi, theta = custom_pose(
        index / eval_size, eval_theta=theta, fix_camera=False, camera_track=camera_track
    )
